...
async function pollCoachResult(expectedAfter, onProgress, maxWaitMs = 180000) {
  const RAW_URL = `https://raw.githubusercontent.com/${GITHUB_REPO}/main/data/coach-latest.json`;
  const interval = 6000;  // check cada 6s
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(check, 8000); // primer check después de 8s
  });
}
